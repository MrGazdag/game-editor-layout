let keyboard: KeyboardLayoutMap | null = null;
...
export default class Keybind {
    ...
    constructor(code: string, action: string, ctrl: boolean=false, shift: boolean=false, alt: boolean=false, meta: boolean=false) {
        this.ctrl = ctrl;
        this.shift = shift;
        this.alt = alt;
        this.meta = meta;
        ...
        let unsafe = false;
        if (unsafeKeybinds) for (let unsafeKeybind of unsafeKeybinds) {
    ...
    private static convertBindToText(bind: Keybind) {
        let str = this.convertKeyToText(bind.getCode());
        if (bind.isMeta()) str = "Meta + " + str;
        if (bind.isAlt()) str = "Alt + " + str;
        if (bind.isShift()) str = "Shift + " + str;
        if (bind.isCtrl()) str = "Ctrl + " + str;

        return str;
    }

    private static convertKeyToText(code: string) {
        if (keyboard) {
